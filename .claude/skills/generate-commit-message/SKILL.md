...
**Architecture** sections for accurate technical detail (build/test commands, package names, architecture) when
describing what changed. Treat it as the single source of truth; do not reinterpret or contradict its rules.
...
2. **Compose the message** in this exact shape:
   ...
   ```
    ...
    - **Summary line**: `<scope>: <Brief description>` — imperative or descriptive, lower-case after the colon, no
      trailing period, ideally <= 72 characters. Use the scope prefix that matches this repo's history (`feat`, `fix`,
