...
The version to summarise (e.g. `7.2.0`) is passed as `args` — if blank, ask the user which version to summarise rather than guessing.

## 🔍 Gather current state

1. Read `documentation/history/PR_DESCRIPTION_v<version>.md`. If it doesn't exist, tell the user to run the `generate-pr-description` skill for that version first, and stop.
